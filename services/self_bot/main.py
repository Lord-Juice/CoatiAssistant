...
def normalize_remote_users(conf):
    seen = set()
    normalized = []
    for user_id in conf.get("remote-users", []):
        uid = str(user_id)
        if uid not in seen:
            seen.add(uid)
            normalized.append(uid)
    conf["remote-users"] = normalized
    # ensure forward config exists
    conf.setdefault("forward", {})
    if conf["forward"].get("target") is None:
        conf["forward"].pop("target", None)


with open("config/config.json", "r") as file:
    config = json.load(file)
    normalize_remote_users(config)
...
async def forward_if_enabled(message):
    ...
    if message.attachments:
        attachment_lines = [f"Anhang: {att.url}" for att in message.attachments]

    lines = [header, body, *attachment_lines]
...
def selfbot_menu(bot):
    ...
    print(f"""\n{Fore.RESET}
                           ...
                           ╚█║████╔╝   ██║   ██║╚██████╔╝       ██║   ╚██████╔╝╚██████╔╝███████╗
                            ╚╝╚═══╝    ╚═╝   ╚═╝ ╚═════╝        ╚═╝    ╚═════╝  ╚═════╝ ╚══════╝\n""".replace('█', f'{b}█{y}'))
    print(f"""{y}------------------------------------------------------------------------------------------------------------------------
{w}raadev {b}|{w} https://github.com/AstraaDev {b}|{w} https://github.com/AstraaDev {b}|{w} https://github.com
{y}------------------------------------------------------------------------------------------------------------------------\n""")
...
async def on_message(message):
    if message.author != bot.user:
        await forward_if_enabled(message)
...
async def remoteuser(ctx, action: str, users: commands.Greedy[discord.User]):
    ...
    if action not in ["ADD", "REMOVE"]:
        await ctx.send("> **[ERROR]**: Use `ADD` or `REMOVE`.", delete_after=5)
        return

    if not users:
        await ctx.send("> **[ERROR]**: Mention at least one user.", delete_after=5)
        return
    ...
    for u in users:
        uid = str(u.id)
        if action == "ADD":
            if uid not in config["remote-users"]:
                config["remote-users"].append(uid)
                changed += 1
        else:  # REMOVE
            if uid in config["remote-users"]:
                config["remote-users"].remove(uid)
                changed += 1

    save_config(config)
    selfbot_menu(bot)
    await ctx.send(f"> **Success**: {action} affected `{changed}` user(s)", delete_after=5)


@bot.command(name="forward")
async def forward_command(ctx, action: str=None, user: discord.User=None):
    await ctx.message.delete()

    if action is None or action.upper() not in ["ADD", "REMOVE"]:
        await ctx.send("> **[ERROR]**: Use `forward ADD <@user>` or `forward REMOVE`.", delete_after=5)
        return

    action = action.upper()
    if action == "ADD":
        if not user:
            await ctx.send("> **[ERROR]**: Mention a user to forward messages to.", delete_after=5)
            return
        config.setdefault("forward", {})
        config["forward"]["target"] = str(user.id)
        save_config(config)
        await ctx.send(f"> Forwarding enabled to {user.mention}.", delete_after=5)
    else:  # REMOVE
        if config.get("forward", {}).get("target"):
            config["forward"]["target"] = None
            save_config(config)
            await ctx.send("> Forwarding disabled.", delete_after=5)
        else:
            await ctx.send("> Forwarding was not enabled.", delete_after=5)
